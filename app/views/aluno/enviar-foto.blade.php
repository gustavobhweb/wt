						...
						<div class="conteudo-form">
							<div style='width:100%'>
								<div id="foto">
									...
									<img data-selected='false' class='userPhoto after-choice' src="{{ URL::to('imagens/' . Auth::user()->matricula . '/temp.png?' . time()) }}" data-src="{{ URL::to('img/aluno/foto.png') }}" ondragstart='return false' onerror="$(this).attr('src', $(this).data('src'))" alt="" title="" width="161" height="215" />
							...
							<form id="form-cadastrar-solicitacao" method="post" enctype="multipart/form-data">
								...
								<fieldset>
									...
									<div class="icon-status emailPrincipal">
										<span>E-mail:</span>
										<input type="email" name="email" value="{{{ $user->email }}}" id="email" placeholder="Nome@dominio.com" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" required="required" title="Informe um e-mail válido"/><div class="glif"><div class="glyphicon glifEmail glyphicon-ok"></div></div>
									</div>
									<div class="icon-status emailPrincipal">
										<span>Confirmação e-mail:</span>
										<input type="email" name="confirmaEmail" value="{{{ $user->email }}}"  id="confirmaEmail" placeholder="Nome@dominio.com" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$"  required="required" title="e-mail não confere"/><div class="glif"><div class="glyphicon glifEmail glyphicon-ok"></div></div>
									</div>
									...
									<div class="icon-status dadosEndereco cep">
										<span>Cep:</span>
										<input value="{{{ $user->cep }}}" type="text" name="cep" id="cep" class="cepInput" placeholder="00000-000" pattern="\d{5}-\d{3}" required="required" /><div class="glif"><div class="glyphicon glyphicon-ok"></div></div>
									...
									<div class="icon-status dadosEndereco uf">
										...
										<select name="uf" id="uf" required="required">
											...
											@foreach($ufs as $uf)
												<option value="{{ $uf->id }}" {{ ($uf->titulo == $user->uf) ? "selected='selected'" : '' }} >{{{ $uf->titulo }}}</option>
											@endforeach
										</select>
									</div>
									<div class="icon-status dadosEndereco cidade">
											<span>Cidade:</span>
											<div class="glif"><div class="glyphicon glifSelect glyphicon-ok" style="margin-left: 207px !important;"></div></div>
									...
									<div class="icon-status dadosEndereco endereco">
										<span>Endereço:</span>
										<input value="{{{ $user->endereco }}}" type="text" name="endereco" required="required" id="endereco" placeholder="Endereco"/><div class="glif"><div class="glyphicon glyphicon-ok"></div></div>
									</div>
									<div class="icon-status dadosEndereco numero">
										<span>Número:</span>
										<input value="{{{ $user->numero }}}" type="text" name="numero" required="required" id="numero" placeholder="Número"/><div class="glif"><div class="glyphicon glifNumero glyphicon-ok"></div></div>
									</div>
									</div>
								...
								<fieldset id="localEntrega">
									<h5>Local de entrega:</h5>
									<div class="icon-status">
										<span>Escolha sua unidade de preferência:</span>
										<ul>
											@foreach($instituicoes as $instituicao)
												<li><input type="radio" {{ ($instituicao->id == $instituicaoPadrao) ? 'checked' : '' }} name="local-entrega" id="local-entrega-{{ $instituicao->id }}" value="{{ $instituicao->id }}" class="radioCheck" /><label for="local-entrega-{{ $instituicao->id }}">{{ $instituicao->nome }}</label></li>
											@endforeach
										</ul>
									</div>
								</fieldset>
								...
								<div class='send-buttons'>
									<label id="privacidade">
										<input type="checkbox" name="privacidade" id="privacidade-ckb"/>
									...
									<div style="width:100%;float:left">
				                    	...
				                    	<button style='float:none' type='button' onclick='document.location.href="{{ URL::to('aluno') }}"' class='btn-cancel-iza'>CANCELAR</button>
				                    </div>
			                    </div>
							</form>
...
@section('scripts')
	...
	{{ HTML::script('js/imgSelect.min.js') }}
    {{ HTML::script('js/fb.js') }}
    {{ HTML::script('js/aluno/enviar_foto.js') }}
    {{ HTML::script('js/aluno/jquery.webcam.js') }}
    {{ HTML::script('js/aluno/webcam.js') }}
    {{ HTML::script('js/wm-modal.js') }}
    {{ HTML::script('js/aluno/valida_form_solicitacao.js') }}
@stop
